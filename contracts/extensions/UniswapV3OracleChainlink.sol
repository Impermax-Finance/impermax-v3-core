...
contract UniswapV3OracleChainlink is IUniswapV3Oracle {
	...
	function oraclePriceSqrtX96(address token0, address token1) external returns (uint256 priceSqrtX96) {
		...
		if (source0 == address(0) || source1 == address(0)) {
			...
			revert("UniswapV3OracleChainlink: UNSUPPORTED_PAIR");
		}
		int256 price0 = AggregatorInterface(source0).latestAnswer();
		int256 price1 = AggregatorInterface(source1).latestAnswer();
		if (price0 <= 0 || price1 <= 0) {
			require(fallbackOracle != address(0), "UniswapV3OracleChainlink: PRICE_CALCULATION_ERROR");
			return IUniswapV3Oracle(fallbackOracle).oraclePriceSqrtX96(token0, token1);
		}
	...
	function _acceptAdmin() external {
		require(msg.sender == pendingAdmin, "UniswapV3OracleChainlink: UNAUTHORIZED");
