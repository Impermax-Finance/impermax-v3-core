...
contract('CSetter', function (accounts) {
	...
	it('permissions check', async () => {
		...
		await expectRevert(collateral._setSafetyMarginSqrt(SAFETY_MARGIN_TEST, {from: user}), 'ImpermaxV3Collateral: UNAUTHORIZED');
		await expectRevert(collateral._setLiquidationIncentive(LIQUIDATION_INCENTIVE_TEST, {from: user}), 'ImpermaxV3Collateral: UNAUTHORIZED');
		await expectRevert(collateral._setLiquidationFee(LIQUIDATION_FEE_TEST, {from: user}), 'ImpermaxV3Collateral: UNAUTHORIZED');
	});
